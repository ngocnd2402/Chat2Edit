...
from source.modules.interfaces.video2 import VideoGenerator
...
class Interpreter:
    ...
    def generate_video(self, selected_image, image, program):
        steps = self.parse_program(program)
        for step in steps:
            _, module, args_dict = self.parse_step("r = " + step)   
            self.call_module_video(module, args_dict, selected_image, image)
        return self.video.output_path  

    def call_module_video(self, module, args_dict, selected_image, image):
        if module == "video_init":           
            self.video = VideoGenerator(selected_image, image)

        if module == "videogen":
            arg_names = ["position", "effect"]
            selected_image, effect = self.get_arg_values(args_dict, arg_names)
            try:
